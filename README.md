...
| **রোগ** | ডায়াবেটিস · হৃদরোগ · উচ্চ রক্তচাপ · কিডনি রোগ · স্ট্রোক |
| **ডেটা** | সিনথেটিক (৬,০০০) + প্রকৃত UCI/Pima ডেটাসেট |
| **ইন্টারফেস** | CLI + Flask PWA (বাংলা/English, ডার্ক মোড, মোবাইল ইনস্টলযোগ্য) |
...
| কিডনি রোগ (Kidney Disease) | 0.846 | 0.933 | 0.105 |
| স্ট্রোক (Stroke) | 0.883 | 0.955 | 0.083 |
...
**প্রকৃত ডেটাসেট (UCI/Pima Indians Diabetes):** Accuracy `0.753` · ROC-AUC `0.807` (৭৬৮ নমুনা) — বাস্তব ডেটা সিনথেটিকের চেয়ে কঠিন, তাই মেট্রিক কম হওয়া প্রত্যাশিত ও সৎ। চালান: `python -m src.real_data`।
...
### 🆕 সম্প্রসারণ
- 🩺 **৫টি রোগ** — ডায়াবেটিস, হৃদরোগ, উচ্চ রক্তচাপ, **কিডনি রোগ ও স্ট্রোক**
- 🌍 **প্রকৃত ডেটাসেট** — UCI/Pima Indians Diabetes ডেটায় প্রশিক্ষণ ও মূল্যায়ন
- 📱 **PWA** — মোবাইল/ডেস্কটপে ইনস্টলযোগ্য · সার্ভিস ওয়ার্কার · অফলাইন অ্যাপ শেল

### মূল ফিচার
- 🩺 **মাল্টি-ডিজিজ** — প্রতি রোগের জন্য আলাদা XGBoost মডেল (config-চালিত, সহজে সম্প্রসারণযোগ্য)
...
- 🌐 Flask ওয়েব অ্যাপ ও JSON API · 💻 CLI · 🐳 Docker · ✅ pytest (১৪ টেস্ট)
...
│   ├── real_data.py     # প্রকৃত UCI/Pima ডেটাসেট প্রশিক্ষণ
│   └── predict.py       # পূর্বাভাস (CLI + প্রোগ্রাম্যাটিক + ব্যাচ + মাল্টি + CI)
├── app/
│   ├── app.py           # Flask ওয়েব অ্যাপ (১৪টি API এন্ডপয়েন্ট)
│   ├── static/          # PWA: manifest, service worker, আইকন
│   └── templates/
│       └── index.html   # বাংলা/English PWA UI, ডার্ক মোড, What-If সিমুলেটর
├── tests/
...
python -m src.real_data        # (ঐচ্ছিক) প্রকৃত Pima ডেটায় প্রশিক্ষণ
```
